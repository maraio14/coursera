...
struct StackItem {
public:
  StackItem(Symbol _name, bool _is_local) 
    : name_(_name), is_local_(_is_local) 
  {;}
  Symbol GetName(){return name_;}
...
struct EmitInfo {
  EmitInfo(CgenNodeP _class, CgenClassTableP _class_table) 
    : class_(_class), class_table_(_class_table)
  {;}
   std::vector<StackItem>& GetMethodStack(){return method_stack_;}
   const CgenNodeP& GetCurrentClass(){return class_;}
   CgenClassTableP& GetClassTable() {return class_table_;}
private:
   CgenNodeP class_;
   CgenClassTableP class_table_;
   std::vector<StackItem> method_stack_;
};
...
public:
   ...
   virtual bool CheckNoExpression() {return false;}
   //TODO: make this pure virtual
   virtual void code(std::ostream& str, EmitInfo _emit_info) {return;}
...
public:
   ...
   Class_ copy_Class_();
   virtual const Features& GetFeatures() override {return features;}
   void dump(ostream& stream, int n);
...
public:
   ...
   virtual void EmitCode(EmitInfo _emit_info, std::ostream& _str);

   virtual Symbol GetName() override {return name;}
...
public:
   ...
   Expression copy_Expression();
   void dump(ostream& stream, int n);
   virtual bool CheckNoExpression() override {return true;}

...
def googlePageRankTeleport(ls_nodes, nodes_count, beta, init_sum, epsilon, stop_iter=sys.maxsize):
    #init_sum = 3
    ...
    while not is_epsilon:
        ...
        for idx in range(0, nodes_count):
            ...
            if idx not in dict_inLinks :
                ...
                if idx == 0:
                    Rnew[idx] = beta * sum_innodes + (1-beta)
                elif idx == 1:
                    Rnew[idx] = beta * sum_innodes #+ (1-beta)
                else:
                    Rnew[idx] = beta * sum_innodes
...
R = googlePageRankTeleport([(0,1),(0, 2), (1,0),  ], 3, .7, 1, .001)
